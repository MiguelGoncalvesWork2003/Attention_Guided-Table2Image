...
def IGTD_absolute_error(source, target, max_step=1000, switch_t=0, val_step=300, min_gain=0.00001, random_state=1,
                        save_folder=None, file_name='', print_interval=1):
    '''
    ...
    for s in range(max_step):
        ...
        if print_interval > 0 and (s % print_interval == 0):
...
def IGTD(source, target, err_measure='abs', max_step=30000, switch_t=0, val_step=300, min_gain=0.00001, random_state=1,
    ...
    if err_measure == 'abs':
        index_record, err_record, run_time = IGTD_absolute_error(source=source,
            ...
            random_state=random_state, save_folder=save_folder, file_name=file_name, print_interval=1)
...
def table_to_image(norm_d, scale, fea_dist_method, image_dist_method, save_image_size, max_step, val_step, normDir,
                   error, switch_t=0, min_gain=0.000001, print_interval=1):
    '''
    ...
    np.save(normDir + "/feature_order.npy", index[min_id])
